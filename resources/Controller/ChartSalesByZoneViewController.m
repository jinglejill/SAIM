//
//  ChartSalesByZoneViewController.m
//  SAIM_TEST
//
...
@interface ChartSalesByZoneViewController ()
{
...
@implementation ChartSalesByZoneViewController
@synthesize hostView;
...
- (void)loadView
{
    ...
    self.navigationController.topViewController.title = @"Sales by Zone";
...
-(void)loadViewProcess
{
    ...
    [_homeModel downloadItems:dbSalesSummaryByPeriod condition:periodCondition];
}
...
{
    {
        //prepare receipt to include postcustomer zone
        float sumValueZoneBkk = 0;
        float sumValueZoneNotBkk = 0;
        NSMutableArray *postcodeList = [SharedPostCode sharedPostCode].postcodeList;
        ...
        NSDate *dtPeriodFrom = [Utility stringToDate:periodCondition.periodFrom fromFormat:@"yyyy-MM-dd HH:mm:ss"];
        NSDate *dtPeriodTo = [Utility stringToDate:periodCondition.periodTo fromFormat:@"yyyy-MM-dd HH:mm:ss"];
        NSPredicate *predicate1 = [NSPredicate predicateWithFormat:@"_dtReceiptDate BETWEEN %@",[NSArray arrayWithObjects:dtPeriodFrom, dtPeriodTo, nil]];
        NSArray *filterArray = [receiptList filteredArrayUsingPredicate:predicate1];
        receiptList = [filterArray mutableCopy];
        for(Receipt *item in receiptList)
        {
            ...
            {
                //bkk
                sumValueZoneBkk += [item.payPrice floatValue];
            }
            else
            {
                //find postcode is in bkk zone
                PostCustomer *postCustomer = [Utility getPostCustomer:customerReceipt.postCustomerID];
                NSPredicate *predicate1 = [NSPredicate predicateWithFormat:@"_postcode = %@",postCustomer.postcode];
                NSArray *filterArray = [postcodeList filteredArrayUsingPredicate:predicate1];
                ...
                {
                    sumValueZoneNotBkk += [item.payPrice floatValue];
                }
            }
        }
        {
            SalesByItemData *salesByItem = [[SalesByItemData alloc]init];
            salesByItem.item = @"Bkk";
            salesByItem.floatSumValue = sumValueZoneBkk;
            salesByItem.floatPercent = sumValueZoneBkk/(sumValueZoneBkk+sumValueZoneNotBkk)*100;
        ...
        {
            SalesByItemData *salesByItem = [[SalesByItemData alloc]init];
            salesByItem.item = @"Upcountry";
            salesByItem.floatSumValue = sumValueZoneNotBkk;
            salesByItem.floatPercent = sumValueZoneNotBkk/(sumValueZoneBkk+sumValueZoneNotBkk)*100;
            salesByItem.floatPercent = roundf(salesByItem.floatPercent/0.1);
            salesByItem.floatPercent = salesByItem.floatPercent * 0.1f;
            [_salesByItemDataList addObject:salesByItem];
        }
