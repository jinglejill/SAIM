//
//  DeliverOrderScanViewController.m
//  SAIM_TEST
//
...
@interface DeliverOrderScanViewController ()
{
...
@implementation DeliverOrderScanViewController
@synthesize selectedRunningPoNo;
@synthesize selectedProductionOrderList;
@synthesize btnBack;
...
- (void)loadView
{
    ...
    }
    {
        ...
        indicatorImage = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
...
-(void)scanQRcode:(NSString *)decryptedMessage
{
    ...
    if([lines count]==3 && [lines[0] isEqualToString:line0Format] && [lines[1] length] == 24 && [lines[2] isEqualToString:@"End"] && [Utility isValidProduct:[Utility getProductWithProductCode:lines[1]] error:&error])
    {
        ...
        //มาเช็คว่าอยู่ใน PoNo นี้หรือไม่
        if(_executeQR)
        {
            ProductName *productName = [ProductName getProductNameWithProduct:productQR];
            ProductionOrder *productionOrder = [ProductionOrder getProductionOrderFromRunningPoNo:selectedRunningPoNo productNameID:productName.productNameID color:productQR.color size:productQR.size fromProductOrderList:selectedProductionOrderList];
            if(!productionOrder || productionOrder.quantityRemaining == 0)
            {
                _executeQR = NO;
                
                dispatch_async(dispatch_get_main_queue(),^ {
                    _lblStatus.textColor = [UIColor redColor];
                    _lblStatus.text = @"Scan product is not in this PO No.";
                } );
                if (_audioPlayer)
                {
                    [_audioPlayer play];
            ...
            {
                productionOrder.quantityRemaining -= 1;
            }
        }
        ...
        if(_executeQR)
        {
            ...
            productQR.status = @"I";
            productQR.eventID = 0;
            productQR.productNameID = productName.productNameID;
            ...
            if([_productExecuteTempList count] == [Utility getNumberOfRowForExecuteSql])
            {
                ...
                [_homeModel insertItems:dbProductAndProductionOrder withData:@[_productExecuteList,[NSString stringWithFormat:@"%ld",selectedRunningPoNo]]];
            }
...
- (IBAction)addProduct:(id)sender {
    ...
    ProductionOrder *productionOrder = [ProductionOrder getProductionOrderFromRunningPoNo:selectedRunningPoNo productNameID:productName.productNameID color:_productScan.color size:_productScan.size fromProductOrderList:selectedProductionOrderList];
    if(!productionOrder || productionOrder.quantityRemaining < [txtQuantity.text integerValue])
    {
        dispatch_async(dispatch_get_main_queue(),^ {
            _lblStatus.textColor = [UIColor redColor];
            _lblStatus.text = @"No scan product/Scan product is not enough in this PO No.";
        } );
    }
    else
    {
        productionOrder.quantityRemaining -= [txtQuantity.text integerValue];
