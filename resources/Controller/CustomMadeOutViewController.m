...
@interface CustomMadeOutViewController ()<UISearchBarDelegate,UISearchControllerDelegate>
{
...
@implementation CustomMadeOutViewController
...
- (void)loadView
{
    ...
    self.navigationItem.title = [NSString stringWithFormat:@"Custom-made out"];
    ...
    [self.navigationItem setRightBarButtonItems:@[btnCMOut]];
...
//    [self loadViewProcess];
    ...
    [_homeModel downloadItems:dbCustomMadeOut condition:@[strPage]];
}
...
//    NSPredicate *predicate1 = [NSPredicate predicateWithFormat:@"(_productType = %@ and _customMadeIn = %@)",@"C",@"1"];
...
//    _postDetailList = [arrSort1 mutableCopy];
    ...
    btnCMOut.enabled = [_postDetailList count]>0;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        header = @[@"CM out",@"No",@"Product",@"Customer name",@"Order date"];
    }
        ...
        switch (item%countColumn) {
            case 0:
            {
                ...
                if([postDetail.editType isEqualToString:@"0"])
                {
                    ...
                    [cell.singleTap removeTarget:self action:@selector(updateCustomMadeOut:)];
                    [cell.singleTap removeTarget:self action:@selector(selectRow:)];
                    [cell.singleTap addTarget:self action:@selector(updateCustomMadeOut:)];
                }
...
- (void) updateCustomMadeOut:(UIGestureRecognizer *)gestureRecognizer
{
    NSInteger countColumn = 5;
...
//    [receiptProductItemList addObject:receiptProductItem];
    [_homeModel updateItems:dbReceiptProductItemUpdateCMIn withData:@[selectedPostDetailList,@"0"]];
...
//    if(self.searchBarActive)
//    {
//        postDetail = self.dataSourceForSearchResult[_selectedIndexPathForRow/countColumn-1];
//    }
//    else
    {
...
-(void)updateButtonShowCountSelect
{
    ...
    {
        {
            btnAction.title = [NSString stringWithFormat:@"CM out(%ld)",countSelect];
        }
    }
    else
    {
        {
            btnAction.title = @"CM out";
        }
    }
}
...
//                [receiptProductItemList addObject: receiptProductItem];
    ...
    float countUpdate = ceil([_selectedPostDetailList count]/itemsPerConnection);
    for(int i=0; i<countUpdate; i++)
    {
        NSInteger startIndex = i * itemsPerConnection;
        NSInteger count = MIN([_selectedPostDetailList count] - startIndex, itemsPerConnection );
        NSArray *subArray = [_selectedPostDetailList subarrayWithRange: NSMakeRange( startIndex, count )];
        [_homeModel updateItems:dbReceiptProductItemUpdateCMIn withData:@[subArray,@"0"]];
    }
